...
/**
 * Create a Stripe payment session for recharge (no cart).
 * POST /store/telecom/recharge/create-stripe-session
 *
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
    try {
        ...
        console.error("[recharge create-stripe-session] Error:", error)
        return res.status(500).json({
